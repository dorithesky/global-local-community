...
def parse_from_field(value):
    if not value:
        return 'Unknown', ''
    if isinstance(value, list):
        first = value[0] if value else {}
        if isinstance(first, dict):
            return first.get('email', 'Unknown'), first.get('name', '')
        return str(first), ''
...
def print_message_summary(message):
    ...
    message = as_plain(message)
    from_addr, from_name = parse_from_field(message.get('from') or message.get('from_'))
    subject = message.get('subject', '(no subject)')
    timestamp = format_timestamp(str(message.get('timestamp', '')))
    preview = (message.get('preview') or message.get('text') or '')[:100]
    ...
    if args.monitor:
        ...
        try:
            ...
            if isinstance(to_field, list):
                to_addrs = ', '.join([addr.get('email', '') if isinstance(addr, dict) else str(addr) for addr in to_field])
            else:
                to_addrs = str(to_field)
            print(f"   To: {to_addrs}")
