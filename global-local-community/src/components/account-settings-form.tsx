...
export function AccountSettingsForm({
  ...
  return (
    <div className="space-y-6">
        ...
        <form action={saveProfileIdentityAction} className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input name="displayName" defaultValue={settings.profile.displayName} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none ring-sky-200 focus:ring" />
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-900">City</label>
              <input name="city" defaultValue={settings.profile.city} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none ring-sky-200 focus:ring" />
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-900">Occupation</label>
              <input name="occupation" defaultValue={settings.profile.occupation} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none ring-sky-200 focus:ring" placeholder="Teacher, designer, student" />
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-900">Origin country</label>
              <input name="originCountry" defaultValue={settings.profile.originCountry} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none ring-sky-200 focus:ring" placeholder="Canada, India, UK" />
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-900">Life stage or visa context</label>
              <input name="lifeStage" defaultValue={settings.profile.lifeStage} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none ring-sky-200 focus:ring" placeholder="Student, teacher, job seeker, spouse visa" />
            </div>
          </div>
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-900">Bio</label>
            <textarea name="bio" defaultValue={settings.profile.bio} className="min-h-28 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none ring-sky-200 focus:ring" />
          </div>
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-900">What do you need most right now?</label>
            <select name="immediateNeed" defaultValue={settings.profile.immediateNeed} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none ring-sky-200 focus:ring">
          ...
          <button type="submit" className="rounded-full bg-sky-600 px-5 py-3 text-sm font-medium text-white hover:bg-sky-700">
            Save profile
          </button>
        </form>
      </section>

      <section className="rounded-3xl border border-cyan-100 bg-gradient-to-br from-white to-cyan-50/40 p-6 shadow-sm">
        <form action={saveNotificationPreferencesAction} className="space-y-4">
          ...
          <div className="space-y-3">
            <label className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm font-medium text-slate-800">
              <span>Get Likes</span>
      ...
      <section id="account-controls" className="rounded-3xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-6 shadow-sm">
