...
export function PostListItem({ post }: { post: PostRecord }) {
  return (
    <Link
      href={`/posts/${post.id}`}
      className="block rounded-3xl border border-[var(--border-subtle)] bg-[var(--surface-primary)] p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:p-4.5"
    >
      ...
      <div className="mt-3 min-w-0">
        <h2 className="text-base font-semibold tracking-tight text-[var(--text-primary)]">{post.title}</h2>
        <p className="mt-1.5 line-clamp-2 text-sm leading-6 text-[var(--text-secondary)]">{post.body}</p>
      </div>
    </Link>
  );
}
