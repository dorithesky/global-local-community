...
import { getSavedPosts, getUserCommentedPosts, getUserLikedPosts } from '@/lib/data';

export default async function ActivityPage() {
  const member = await getCurrentMember();
  if (!member) redirect('/');

  const [savedPosts, likedPosts, commentedPosts] = await Promise.all([
    getSavedPosts(),
    getUserLikedPosts(),
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-1 text-2xl font-semibold text-slate-950">{commentedPosts.length}</p>
          </div>
        </div>
      </section>
